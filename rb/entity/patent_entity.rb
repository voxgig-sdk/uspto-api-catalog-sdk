...
class PatentEntity
  ...
  def make
    opts = @_entopts.dup
    PatentEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = UsptoApiCatalogHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Patent, Hash] the current Patent data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = UsptoApiCatalogHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Patent fields)
  def match_get
  ...
  #
  # @param reqmatch [PatentLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Patent.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Patent, Hash] the loaded Patent; raises UsptoApiCatalogError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = UsptoApiCatalogHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PatentListMatch, Hash, nil] match filter (any subset of
  #   Patent fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Patent>, Array] the matching Patent items; raises UsptoApiCatalogError on failure
