...
class TrademarkEntity
  ...
  def make
    opts = @_entopts.dup
    TrademarkEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Trademark.load works with no args).
  ...
  #
  # @param reqmatch [TrademarkListMatch, Hash, nil] match filter (any subset of
  #   Trademark fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
