...
module UsptoApiCatalogUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = UsptoApiCatalogResponse.new({})
    ...
    if fetch_err
      ...
      response = UsptoApiCatalogResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = UsptoApiCatalogResponse.new(fetched)
    else
