# UsptoApiCatalog Ruby SDK Reference

Complete API reference for the UsptoApiCatalog Ruby SDK.
...
#### `UsptoApiCatalogSDK.test(testopts = nil, sdkopts = nil)`
...
Create a new `Patent` entity instance. Pass `nil` for no initial data.

#### `Trademark(data = nil)`

Create a new `Trademark` entity instance. Pass `nil` for no initial data.
...
```ruby
results, err = client.Patent.list(nil)
```
...
```ruby
result, err = client.Patent.load({ "id" => "patent_id" })
```
...
```ruby
results, err = client.Trademark.list(nil)
```
...
```ruby
result, err = client.Trademark.load({ "id" => "trademark_id" })
```
