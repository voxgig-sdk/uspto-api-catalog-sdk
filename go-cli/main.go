// Command UsptoApiCatalog-cli is a CLI + REPL for the UsptoApiCatalog SDK,
...
const prompt = "uspto-api-catalog"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "patent trademark"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      USPTO_API_CATALOG_APIKEY (api key), USPTO_API_CATALOG_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: USPTO_API_CATALOG_APIKEY carries the API key and
	// USPTO_API_CATALOG_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("USPTO_API_CATALOG_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("USPTO_API_CATALOG_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewUsptoApiCatalogSDK(opts)
