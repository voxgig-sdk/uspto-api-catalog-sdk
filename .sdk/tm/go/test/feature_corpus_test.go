...
//
// NewUsptoApiCatalogSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.UsptoApiCatalogSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewUsptoApiCatalogSDK(opts)
}
...
func fcCandidates(client *sdk.UsptoApiCatalogSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.UsptoApiCatalogSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.UsptoApiCatalogSDK, name string) any {
