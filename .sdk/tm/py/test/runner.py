# UsptoApiCatalog SDK test runner
...
class UsptoApiCatalogTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            UsptoApiCatalogTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = UsptoApiCatalogTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = UsptoApiCatalogTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = UsptoApiCatalogTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = UsptoApiCatalogTestRunner.getenv(key)
        ...
        explain = UsptoApiCatalogTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if UsptoApiCatalogTestRunner._test_control is not None:
            return UsptoApiCatalogTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                UsptoApiCatalogTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            UsptoApiCatalogTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = UsptoApiCatalogTestRunner.load_test_control()
...
def load_env_local():
    UsptoApiCatalogTestRunner.load_env_local()
...
def entity_list_to_data(lst):
    return UsptoApiCatalogTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return UsptoApiCatalogTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return UsptoApiCatalogTestRunner.load_test_control()


def live_client_options():
    return UsptoApiCatalogTestRunner.live_client_options()


def live_delay_ms():
    return UsptoApiCatalogTestRunner.live_delay_ms()
