...
// UsptoApiCatalog SDK retry feature
...
class UsptoApiCatalogRetryFeature extends UsptoApiCatalogBaseFeature
{
    ...
    public function init(UsptoApiCatalogContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (UsptoApiCatalogContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(UsptoApiCatalogContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(UsptoApiCatalogContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
