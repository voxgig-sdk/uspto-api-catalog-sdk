...
// UsptoApiCatalog SDK feature test
//
...
class FtCtrl extends UsptoApiCatalogControl
{
...
class FtClient
{
    ...
    private UsptoApiCatalogUtility $utility;

    public function __construct(UsptoApiCatalogUtility $utility, array $options)
    {
    ...
    public function get_utility(): UsptoApiCatalogUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (UsptoApiCatalogContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public UsptoApiCatalogContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (UsptoApiCatalogContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = UsptoApiCatalogConfig::make_feature($name);
    ...
    private static function buildUrl(UsptoApiCatalogSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof UsptoApiCatalogError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new UsptoApiCatalogResponse($fetched);
        }
    ...
    private function fail(UsptoApiCatalogContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (UsptoApiCatalogContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
