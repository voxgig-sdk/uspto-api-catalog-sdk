...
import (
	...
	omni "github.com/voxgig-sdk/uspto-api-catalog-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.UsptoApiCatalogError); is {
				out := map[string]any{
					"name":    "UsptoApiCatalogError",
...
func omniCtx(arg any, client *sdk.UsptoApiCatalogSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.UsptoApiCatalogSDK); is {
			client = live
