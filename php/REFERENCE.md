# UsptoApiCatalog PHP SDK Reference

Complete API reference for the UsptoApiCatalog PHP SDK.
...
#### `UsptoApiCatalogSDK::test($testopts = null, $sdkopts = null)`
...
Create a new `PatentEntity` instance. Pass `null` for no initial data.
...
```php
[$results, $err] = $client->Patent()->list([]);
```
...
```php
[$results, $err] = $client->Trademark()->list([]);
```
