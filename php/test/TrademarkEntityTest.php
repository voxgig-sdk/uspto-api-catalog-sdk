...
class TrademarkEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = UsptoApiCatalogSDK::test(null, null);
        $ent = $testsdk->Trademark(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = UsptoApiCatalogSDK::test($seed, null);
        $seen = iterator_to_array($base->Trademark(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = UsptoApiCatalogConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = UsptoApiCatalogSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Trademark(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = trademark_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "trademark." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set USPTO_API_CATALOG_TEST_TRADEMARK_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $trademark_ref01_data = null;
        if (count($trademark_ref01_data_raw) > 0) {
...
function trademark_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/trademark/TrademarkTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["trademark01", "trademark02", "trademark03"] as $k) {
    ...
    $entid_env_raw = getenv("USPTO_API_CATALOG_TEST_TRADEMARK_ENTID");
