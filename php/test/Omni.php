...
//    only - a typed UsptoApiCatalogContext there would make every ctx
...
final class UsptoApiCatalogOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new UsptoApiCatalogContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new UsptoApiCatalogSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new UsptoApiCatalogResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new UsptoApiCatalogError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new UsptoApiCatalogResponse($clean['response']);
    ...
    public static function err_from_map($m): ?UsptoApiCatalogError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new UsptoApiCatalogError(is_string($code) ? $code : '', $msg);
    }
