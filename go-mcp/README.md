# uspto-api-catalog-mcp

[MCP](https://modelcontextprotocol.io) server exposing the UsptoApiCatalog SDK as
two agent tools — `uspto-api-catalog_list` and `uspto-api-catalog_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/uspto-api-catalog-mcp)
make build
...
claude mcp add --scope user uspto-api-catalog \
  -- /absolute/path/to/uspto-api-catalog-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./uspto-api-catalog-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// uspto-api-catalog_list: first page of records
{ "entity": "patent" }
{ "entity": "patent", "query": { } }

// uspto-api-catalog_load: one record by id
{ "entity": "patent", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/uspto-api-catalog-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user uspto-api-catalog \
     -- "$PWD"/dist/*/uspto-api-catalog-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `uspto-api-catalog_list` and `uspto-api-catalog_load` tools now appear
   in new sessions. Ask the agent to *"list patent using uspto-api-catalog"*
   and it calls `uspto-api-catalog_list` with `{"entity":"patent"}`.
...
### Call the `uspto-api-catalog_list` tool
...
### Call the `uspto-api-catalog_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `uspto-api-catalog_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `uspto-api-catalog_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./uspto-api-catalog-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"uspto-api-catalog_load","arguments":{"entity":"patent","query":{"id":1}}}}'
```
