...
//     (\`new UsptoApiCatalogSDK(...)\` / \`UsptoApiCatalogSDK.test(...)\` become
//     \`UsptoApiCatalogSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'UsptoApiCatalogSDK'
...
const TEST_SEED = {"entity":{"patent":{"test01":{"id":"test01"}},"trademark":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new UsptoApiCatalogSDK(...)\` and any
// \`UsptoApiCatalogSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+UsptoApiCatalogSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/UsptoApiCatalogSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
