...
describe('TrademarkDirect', async () => {
  ...
  test('direct-load-trademark', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-trademark', setup.live)) return
  ...
  test('direct-list-trademark', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-trademark', setup.live)) return
    const { client, calls } = setup
