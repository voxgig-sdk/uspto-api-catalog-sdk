...
describe("TrademarkDirect", function()
  it("should direct-list-trademark", function()
    local setup = trademark_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-trademark", setup.live and "live" or "unit")
  ...
  it("should direct-load-trademark", function()
    local setup = trademark_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-trademark", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function trademark_direct_setup(mockres)
