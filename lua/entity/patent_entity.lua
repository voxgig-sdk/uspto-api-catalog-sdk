...
local PatentEntity = {}
PatentEntity.__index = PatentEntity


function PatentEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PatentEntity)
  self._name = "patent"
  self._client = client
...
function PatentEntity:get_name()
  return self._name
end


function PatentEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PatentEntity.new(self._client, opts)
end
...
function PatentEntity:mark_deleted()
  self._deleted = true
end


function PatentEntity:deleted()
  return true == self._deleted
end


function PatentEntity:data_set(args)
  if args ~= nil then
...
function PatentEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PatentEntity:match_set(args)
  if args ~= nil then
...
function PatentEntity:match_get()
...
function PatentEntity:stream(action, args, callopts)
...
---@param reqmatch PatentLoadMatch
---@param ctrl? table
---@return Patent
---@return string? err
function PatentEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch PatentListMatch
---@param ctrl? table
---@return Patent[]
---@return string? err
function PatentEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function PatentEntity:_run_op(ctx, post_done)
  local utility = self._utility
