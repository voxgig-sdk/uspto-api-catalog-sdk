...
require("uspto-api-catalog_types")
...
local UsptoApiCatalogSDK = {}
UsptoApiCatalogSDK.__index = UsptoApiCatalogSDK
...
UsptoApiCatalogSDK._make_feature = _make_feature


function UsptoApiCatalogSDK.new(options)
  local self = setmetatable({}, UsptoApiCatalogSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function UsptoApiCatalogSDK:options_map()
...
function UsptoApiCatalogSDK:get_utility()
  return Utility.copy(self._utility)
end


function UsptoApiCatalogSDK:get_root_ctx()
  return self._rootctx
end


function UsptoApiCatalogSDK:prepare(fetchargs)
...
function UsptoApiCatalogSDK:direct(fetchargs)
...
function UsptoApiCatalogSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function UsptoApiCatalogSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "UsptoApiCatalogSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function UsptoApiCatalogSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function UsptoApiCatalogSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "UsptoApiCatalogSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function UsptoApiCatalogSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = UsptoApiCatalogSDK.new(sdkopts)
  sdk.mode = "test"
