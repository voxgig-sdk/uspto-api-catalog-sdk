# UsptoApiCatalog Lua SDK



The Lua SDK for the UsptoApiCatalog API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Patent()` — each with the same small set of operations (`list`, `load`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
local patent, err = client:Patent():load()
if err then error(err) end
print(patent)
```
...
```lua
local trademark, err = client:Trademark():load()
```
...
```
lua/
├── uspto-api-catalog_sdk.lua    -- Main SDK module
...
The main module (`uspto-api-catalog_sdk`) exports the SDK constructor
