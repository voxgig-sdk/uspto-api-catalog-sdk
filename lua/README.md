...
The Lua SDK for the UsptoApiCatalog API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Patent(nil):list(nil, nil)
...
```lua
local result, err = client:Patent(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:UsptoApiCatalog(nil):load(
  { id = "test01" }, nil
)
