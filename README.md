...
The USPTO API Catalog provides developers with access to various APIs related to intellectual property.

Learn more about USPTO API Catalog at [developer.uspto.gov/api-catalog](https://developer.uspto.gov/api-catalog).
...
This SDK exposes the API as a small set of **semantic entities** — Patent and Trademark — that you
...
const items = await client.Patent().list()
```
...
```go
client := sdk.Test()
result, err := client.Patent(nil).List(
    nil, nil,
)
```
...
$patents = $client->Patent()->list();
print_r(array_map(fn($item) => $item->data_get(), $patents));
